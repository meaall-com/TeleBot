...
"""نجح في تغيير السيرة الذاتية لملفي الشخصي : .pbio, .ppic, .pname, .delpfp {n}"""
...
async def _(event):
    ...
    try:
        ...
        await event.edit("نجح في تغيير السيرة الذاتية لملفي الشخصي")
...
async def _(event):
    ...
    await event.edit("تحميل صورة الملف الشخصي على بلدي المحلي ...")
...
async def remove_profilepic(delpfp):
    """ بالنسبة لأمر .delpfp ، احذف صورة ملف التعريف الحالية في Telegram. """
    group = delpfp.text[8:]
    if group == 'all':
        lim = 0
    elif group.isdigit():
    ...
    await delpfp.edit(
        f"`تم حذف {len (input_photos)} صورة (صور) الملف الشخصي بنجاح.`")
